...
int main(int argc, char* argv[]){
	...
	while(1){
		...
		switch(req.packetType){
			case COMMAND_CD:
				ret = requestCd(sfd, &req, prompt.curDir);
				if(-1==ret){printf("arguments invalid\n");}break;
			case COMMAND_LS:break;
			case COMMAND_PUTS:break;
			case COMMAND_GETS:break;
			case COMMAND_REMOVE:break;
			case COMMAND_PWD:break;
			default:break;	
		}
	}
	return 0;	
